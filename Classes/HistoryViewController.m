...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (indexPath.row > 0) {
		...
		if ([h.type isEqualToString:@"nutrition.png"]) {
				cell.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
		}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (indexPath.row == 0) {
		...
		if ([h.type isEqualToString:@"nutrition.png"]) {
				PortionHistoryViewController *portionHistoryViewController = [[PortionHistoryViewController alloc]
																		initWithNibName:@"PortionHistoryViewController"
																		bundle:nil];
				portionHistoryViewController.passedDate = [dateList objectAtIndex:indexPath.section];
				Bring_ItAppDelegate *delegate = [[UIApplication sharedApplication] delegate];
				[delegate.historyNavController pushViewController:portionHistoryViewController animated:YES];
				[portionHistoryViewController release];
		} 
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
	...
	NSTimeInterval interval = [date timeIntervalSinceDate:[BIUtility dateFromString:p.startDate]];
